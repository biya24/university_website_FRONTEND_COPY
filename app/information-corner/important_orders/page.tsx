const TITLE = "Orders / Circulars";

async function fetchOrders() {
  const res = await fetch(
    "https://departments.ssus.ac.in/api/website/category/important_orders",
    { cache: "no-store" }
  );
        ...
        <article className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-6 md:p-10">
            <div className="drupal-body prose prose-lg max-w-none">
              {orders.length === 0 ? (
                <p>No orders or circulars available at present.</p>
              ) : (
                <table>
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {orders.map((o: any) => (
                      <tr key={o.id}>
                        ...
                        <td>{o.published_date_formatted}</td>
                        <td>{o.deadline_date_formatted}</td>
                        <td>
                          {o.file_url ? (
                            <a
                              href={o.file_url}
